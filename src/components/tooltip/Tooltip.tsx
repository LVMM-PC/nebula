...
export default class Tooltip extends Vue {
  ...
  getDisabledCompatibleChildren(ele) {
    const isAntBtn =
      ele.componentOptions && ele.componentOptions.Ctor.options.__ANT_BUTTON;
    if (
      (isAntBtn &&
      ...
      const spanStyle = {
        ...
        cursor: "not-allowed",
        width:
          isAntBtn && ele.componentOptions.propsData.disabled ? "100%" : null
      };
      const buttonStyle = {
        ...omitted,
        pointerEvents: "none"
  ...
  render(h) {
    ...
    let children = ($slots.default || []).filter(
      c => c.tag || c.text.trim() !== ""
    );
    // @ts-ignore
