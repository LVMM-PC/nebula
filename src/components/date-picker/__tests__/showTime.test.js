...
describe("DatePicker with showTime", () => {
  ...
  it("should have correct className when use12Hours is true", async () => {
    ...
    await asyncExpect(() => {
      expect(
        calendarWrapper.findAll(".ant-calendar-time-picker-column-4").length
      ).toBe(0);
    });
    calendarWrapper
...
describe("RangePicker with showTime", async () => {
  it("should trigger onChange when select value", async () => {
    ...
    await asyncExpect(() => {
      expect(
        calendarWrapper.find(".ant-calendar-time-picker-btn").classes()
    ...
    await asyncExpect(() => {
      ...
      expect(
        calendarWrapper.find(".ant-calendar-ok-btn").classes()
