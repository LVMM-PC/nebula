...
describe("RangePicker", () => {
  ...
  it("highlight range when hover presetted range", async () => {
    ...
    await asyncExpect(() => {
      expect(
        rangeCalendarWrapper.findAll(".ant-calendar-selected-day").length
      ).toBe(2);
    });
  });
  ...
  describe("preset range", () => {
    ...
    it("static range", async () => {
      ...
      await asyncExpect(() => {
        expect(
          wrapper.findAll(".ant-calendar-range-picker-input").at(0).element
            .value
        ).toBe(range[0].format(format));
      });
      await asyncExpect(() => {
        const inputs = wrapper.findAll(".ant-calendar-range-picker-input");
        expect(inputs.at(inputs.length - 1).element.value).toBe(
