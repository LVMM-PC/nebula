...
function onInputSelect(direction, value) {
  ...
  if (
    ...
    selectedValue[1 - index] = this.showTimePicker
      ? selectedValue[index]
      : undefined;
  }
...
const RangeCalendar = {
  props: {
    ...
    mode: PropTypes.arrayOf(
      PropTypes.oneOf(["date", "month", "year", "decade"])
    ),
  ...
  methods: {
    ...
    onDayHover(value) {
      ...
      if (type === "start" && sSelectedValue[1]) {
        hoverValue =
          this.compare(value, sSelectedValue[1]) < 0
            ? [value, sSelectedValue[1]]
            : [value];
      } else if (type === "end" && sSelectedValue[0]) {
        hoverValue =
          this.compare(value, sSelectedValue[0]) > 0
    ...
    getEndValue() {
      ...
      const endValue = sValue[1]
        ? sValue[1].clone()
        : sValue[0].clone().add(1, "month");
      // keep selectedTime when select date
    ...
    getEndDisableTime() {
      ...
      const startValue =
        (sSelectedValue && sSelectedValue[0]) || sValue[0].clone();
    ...
    fireSelectValueChange(selectedValue, direct) {
      ...
      if (!this.sSelectedValue[0] || !this.sSelectedValue[1]) {
        ...
        const endValue =
          selectedValue[1] || startValue.clone().add(1, "months");
        this.setState({
          sSelectedValue: selectedValue,
          sValue:
