...
@Component({
  ...
  watch: {
    __propsSymbol__() {
      Object.assign(
        this.$data,
        this.getDerivedStateFromProps(getOptionProps(this), this.$data)
      );
    }
    // value (val) {
...
class Select extends Vue {
  ...
  public saveInputRef: any;
  public saveInputMirrorRef: any;
  public saveTopCtrlRef: any;
  public saveSelectTriggerRef: any;
  public saveRootRef: any;
  public saveSelectionRef: any;
  ...
  created() {
    ...
    // @ts-ignore
    let propsSymbol = this.__propsSymbol__;
    warning(
      propsSymbol,
      "Replace slots.default with props.children and pass props.__propsSymbol__"
    );
    this._value = this.getValueFromProps(props, true); // true: use default value
    this._inputValue = props.combobox
      ...
      : "";
    this._open = props.defaultOpen;
    this._optionsInfo = optionsInfo;
    // a flag for aviod redundant getOptionsInfoFromProps call
    this._skipBuildOptionsInfo = true;
  }

  beforeMount() {
    const state = this.getDerivedStateFromProps(
      getOptionProps(this),
      this.$data
    );
  ...
  getInputValueForCombobox(props, optionsInfo, useDefaultValue = null) {
    let value = [];
  ...
  getOptionsInfoFromProps(props, preState = null) {
  ...
  getValueFromProps(props, useDefaultValue = null) {
    let value = [];
    if ("value" in props && !useDefaultValue) {
      value = toArray(props.value);
    }
  ...
  getOptionInfoBySingleValue(value, optionsInfo = null) {
    let info;
  ...
  getLabelBySingleValue(value, optionsInfo = null) {
  ...
  setOpenState(open, needFocus = null) {
    const { $props: props, $data: state } = this;
    if (state._open === open) {
  ...
  maybeFocus(open, needFocus = null) {
    if (needFocus || open) {
  ...
  removeSelected(selectedKey, e = null) {
    const props = this.$props;
  ...
  fireChange(value) {
    ...
    this.$emit(
      "change",
      vls,
      isMultipleOrTags(this.$props) ? options : options[0]
    );
  }
  ...
  renderTopControlNode() {
    ...
    if (isSingleMode(props)) {
      ...
      if (isMultipleOrTags(props) && choiceTransitionName) {
        ...
        innerNode = (
          <transition-group {...transitionProps}>
            {selectedValueNodes}
          </transition-group>
        );
      } else {
